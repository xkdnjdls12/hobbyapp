...
public class GamePost2 extends AppCompatActivity {
    ...
    EditText editText;
    TextView textView;
    List<String> commentsListg; // 댓글 목록을 저장하는 리스트
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_game_post2);
        ...
        btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!comment.isEmpty()) {
                    commentsListg.add(comment);
                    updateCommentsTextView();
                    Toast.makeText(GamePost2.this, "댓글이 작성되었습니다.", Toast.LENGTH_SHORT).show();
                }
                editText.setText("");
            }
        });
        ...
        mStore.collection(FirebaseID.gamepost).document(id)
                .get()
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()) {
                            if (task.getResult().exists()) {
                                ...
                                Toast.makeText(GamePost2.this, "삭제된 문서입니다.", Toast.LENGTH_SHORT).show();
                            }
                        }
                    }
                });

        commentsListg = new ArrayList<>();
        ...
        buttonFavorite.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(GamePost2.this, LikeBoard.class);
                intent.putExtra("title", mTitleText.getText().toString());
                intent.putExtra("contents", mContentsText.getText().toString());
                Toast.makeText(GamePost2.this, "게시글이 찜 되었습니다.", Toast.LENGTH_SHORT).show();
    ...
    private void updateCommentsTextView() {
        ...
        for (String comment : commentsListg) {
    ...
    private void saveState() {
        ...
        editor.putString("commentsListg", commentsListToString());
        editor.commit();
    }

    private void restoreState() {
        SharedPreferences pref = getSharedPreferences("pref", Activity.MODE_PRIVATE);
        if ((pref != null) && (pref.contains("comment")) && (pref.contains("commentsListg"))) {
            String comment = pref.getString("comment", "");
            String commentsListString = pref.getString("commentsListg", "");
            editText.setText(comment);
            commentsListg = stringToCommentsList(commentsListString);
            updateCommentsTextView();
        }
    }
        ...
        builder.setMessage("해당 댓글을 삭제하시겠습니까?")
                .setPositiveButton("확인", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        deleteComment();
                        Toast.makeText(GamePost2.this, "댓글이 삭제되었습니다.", Toast.LENGTH_SHORT).show();
                    }
                })
    ...
    private void deleteComment() {
        String clickedComment = commentsListg.get(commentsListg.size() - 1);
        commentsListg.remove(clickedComment);
        updateCommentsTextView();
    }
